...
int findInString(string s, string find){
    // finds the occurances of a word in a string
    int pos = s.find(find);
    int count = stoi(s.substr(pos - 3, 2));
    while (pos != string::npos){
        pos = s.find(find, pos + 1);
        if (pos == -1)
            break;
        int newCount = stoi(s.substr(pos - 3, 2));
        if (newCount > count)
            count = newCount;
    }
    return count;
...
int main(){
    ...
    while (getline(ifs, line)){
        int blue = findInString(line, "blue");
        int red = findInString(line, "red");
        int green = findInString(line, "green");
        if (red <= 12 && green <= 13 && blue <= 14){
            sum += stoi(line.substr(5, 3));
            cout << stoi(line.substr(5, 3)) << endl;
        }
    }
    cout << "Sum: " << sum;
